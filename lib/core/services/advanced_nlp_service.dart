...
class AdvancedNlpService {
  ...
  Future<ParsedVoiceInput> parseVoiceInput(String text) async {
    ...
    if (basicResult.parsedDate == null ||
        (basicResult.dateConfidence ?? 0) < 0.7) {
      ...
      for (final expression in dateExpressions) {
        try {
          ...
          if (expression.contains('next') ||
              ...
              expression.contains('tomorrow') ||
              expression.contains('today')) {
            ...
            if (expression.contains('this weekend')) {
              ...
              return jiffy
                  .add(days: daysUntilSaturday > 0 ? daysUntilSaturday : 7)
                  .dateTime;
            }
          }
  ...
  DateTime? _parseRelativeExpression(String expression) {
    ...
    final fromNowPattern =
        RegExp(r'(\d+)\s+(days?|weeks?|months?)\s+from\s+now');
  ...
  DateResult? _parseEnhancedDatePatterns(String text) {
    ...
    if (normalizedText.contains('next weekend')) {
      final nextSaturday =
          jiffy.add(days: 6 - jiffy.dateTime.weekday + 7).dateTime;
