...
class MessageParserService {
  ...
  String _preprocessMessage(String text) {
    ...
    text = text.replaceAll(
        RegExp(r'\[.*?\]'), ''); // Remove [timestamp] or [attachments]

    return text;
  }

  /// Parse direct requests and commands
  Future<List<ExtractedTask>> _parseDirectRequests(
      String text, SharedContent content) async {
    final tasks = <ExtractedTask>[];
  ...
  Future<List<ExtractedTask>> _parseScheduledItems(
      String text, SharedContent content) async {
    final tasks = <ExtractedTask>[];
  ...
  Future<List<ExtractedTask>> _parseShoppingLists(
      String text, SharedContent content) async {
    final tasks = <ExtractedTask>[];
      ...
      for (final match in matches) {
        ...
        for (final item in items) {
          ...
          final task = ExtractedTask(
            ...
            extractedTime: dateTime['time'],
            suggestedCategory:
                'household', // Shopping items default to household
  ...
  Future<List<ExtractedTask>> _parseAppointments(
      String text, SharedContent content) async {
    final tasks = <ExtractedTask>[];
    ...
    for (final pattern in appointmentPatterns) {
      ...
      for (final match in matches) {
        ...
        final appointmentInfo =
            match.group(2)?.trim() ?? match.group(1)?.trim() ?? '';
  ...
  String _cleanTaskTitle(String title) {
    ...
    title = title.replaceAll(
        RegExp(r'\s+(today|tomorrow|next week|this \w+).*$',
            caseSensitive: false),
        '');
    title = title.trim();
  ...
  List<ExtractedTask> _scoreAndRankTasks(
      List<ExtractedTask> tasks, SharedContent content) {
  ...
  Future<List<ExtractedTask>> _parseReminders(
      String text, SharedContent content) async {
    final tasks = <ExtractedTask>[];
    ...
    for (final pattern in reminderPatterns) {
      ...
      for (final match in matches) {
        ...
        final task = ExtractedTask(
          ...
          suggestedCategory: _suggestCategory(actionPart),
          confidence: _calculateConfidence(fullMatch, actionPart) +
              0.1, // Boost for explicit reminders
          source: TaskSource.chat,
          conversationContext: content.conversationContext,
          senderInfo: content.senderInfo,
          keywords: _extractKeywords(actionPart),
  ...
  Future<List<ExtractedTask>> _parseDeadlines(
      String text, SharedContent content) async {
    final tasks = <ExtractedTask>[];
    ...
    for (final pattern in deadlinePatterns) {
      ...
      for (final match in matches) {
        ...
        final task = ExtractedTask(
          ...
          confidence: _calculateConfidence(fullMatch, taskPart) +
              0.15, // Higher boost for deadlines
          source: TaskSource.chat,
          conversationContext: content.conversationContext,
          senderInfo: content.senderInfo,
          keywords: _extractKeywords(taskPart),
          inferredPriority:
              TaskPriority.high, // Deadlines are typically high priority
        );
  ...
  Future<List<ExtractedTask>> _parseActionItems(
      String text, SharedContent content) async {
    final tasks = <ExtractedTask>[];
  ...
  Future<List<ExtractedTask>> _parseHouseholdTasks(
      String text, SharedContent content) async {
    final tasks = <ExtractedTask>[];
    ...
    for (final pattern in householdPatterns) {
      ...
      for (final match in matches) {
        ...
        final task = ExtractedTask(
          ...
          extractedTime: dateTime['time'],
          suggestedCategory:
              'household', // All household tasks get this category
          confidence: _calculateConfidence(fullMatch, taskPart) +
              0.05, // Small boost for household context
          source: TaskSource.chat,
          conversationContext: content.conversationContext,
          senderInfo: content.senderInfo,
          keywords: _extractKeywords(taskPart),
  ...
  double _calculateConfidence(String originalText, String extractedPart) {
    ...
    if (_hasQuestionIndicators(lowerOriginal))
      confidence -= 0.2; // Questions less likely to be tasks

    // Context bonuses
    if (_hasUrgencyIndicators(lowerOriginal)) confidence += 0.1;
    if (_hasPersonalPronouns(lowerOriginal))
      confidence += 0.05; // "I need to", "you should"
